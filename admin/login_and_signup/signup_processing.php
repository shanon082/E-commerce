...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
        ...
        if ($password !== $confirmPassword) {
            ...
            $sql = "SELECT * FROM admindetails WHERE username = :username OR email = :email";
            ...
            if ($existingUser) {
                ...
                $sql = "INSERT INTO admindetails (username, phone, email, password) VALUES (:username, :phone, :email, :password);";
                $stmt = $conn -> prepare($sql);
                $stmt -> bindParam(":username", $username);
